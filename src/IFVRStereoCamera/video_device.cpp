...
#define qDebug(fmt, arg...) printf(fmt, ##arg)

static int verbose = 1;
//#define pr_debug(fmt, arg...) \
//    if (verbose) printf(fmt, ##arg)

#define pr_debug(fmt, arg...) printf(fmt, ##arg)

#define CLEAR(x) memset(&(x), 0, sizeof(x))

video_device::video_device(std::string devn, int camera_width, int camera_hight)
{
    int ret, dev;

    //QByteArray ba = devn.toLocal8Bit();
    const char *c_str = devn.c_str();

    this->dev_name = c_str;
    this->image_width = camera_width;
    this->image_hight = camera_hight;
    this->nbufs = 6;
    this->fd = -1;
...
int video_device::yuyv_2_y_gray(const void *p, int size, unsigned char *frame_buffer)
{
    ...
    for(int i = 0; i< (size/2); i++){
    	frame_buffer[i] = pointer[i*2];
    }
	return 0;
}


int video_device::yuyv_2_lr_gray(const void *p, int size, unsigned char *l_buffer, unsigned char *r_buffer)
{
