...
public:
    ...
    explicit video_device(std::string dev_name, int camera_width, int camera_hight);
    ...
    int yuyv_2_y_gray(const void *p, int size, unsigned char *frame_buffer);
    int yuyv_2_lr_gray(const void *p, int size, unsigned char *l_buffer, unsigned char *r_buffer);
    ~video_device();
